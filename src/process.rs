...
pub fn process_image(context:&ProcessOptions) -> error::Result<RgbImage> {
    ...
    let mut raw_image = match rawimage::RawImage::new_from_image_with_decompand(&context.input, md.sample_bit_mode_id) {
        Ok(img) => img,
        Err(why) => return Err(why)
    };
    ...
    if juno_config.defaults.apply_hot_pixel_correction {
        ...
        match raw_image.apply_hot_pixel_correction(juno_config.defaults.hpc_window_size, juno_config.defaults.hpc_threshold) {
            Ok(_) => {},
            Err(why) => return Err(why)
        };
    }
    ...
    if juno_config.defaults.apply_weights {
        vprintln!("Applying channel weight multiples ({}, {}, {} X R, G, B)...", context.red_weight, context.green_weight, context.blue_weight);
        match raw_image.apply_weights(context.red_weight, context.green_weight, context.blue_weight) {
            Ok(_) => {},
            Err(why) => return Err(why)
        };
    }
    ...
    let lens: Box<dyn Lens> = match context.lens {
        ...
        SupportedLens::Fisheye => Box::new(FisheyeEquisolidLens::new(cyl_map.width, cyl_map.height, 13.0, context.fov))
    };
