...
pub  fn load_mask(camera:enums::Camera) -> error::Result<ImageBuffer> {
    ...
    match camera {
        enums::Camera::RED => 
                Ok(MASK_CACHE.lock().unwrap().check_red(&path::locate_calibration_file(&c.calibration.inpaint_red).unwrap()).unwrap()),
        enums::Camera::GREEN => 
                Ok(MASK_CACHE.lock().unwrap().check_green(&path::locate_calibration_file(&c.calibration.inpaint_green).unwrap()).unwrap()), 
        enums::Camera::BLUE => 
                Ok(MASK_CACHE.lock().unwrap().check_blue(&path::locate_calibration_file(&c.calibration.inpaint_blue).unwrap()).unwrap()),
...
pub fn load_flat_file(camera:enums::Camera) -> error::Result<ImageBuffer> {
    ...
    let flat = match camera {
        enums::Camera::RED => 
                Ok(FLAT_CACHE.lock().unwrap().check_red(&path::locate_calibration_file(&c.calibration.flat_red).unwrap()).unwrap()),
        enums::Camera::GREEN => 
                Ok(FLAT_CACHE.lock().unwrap().check_green(&path::locate_calibration_file(&c.calibration.flat_green).unwrap()).unwrap()), 
        enums::Camera::BLUE => 
                Ok(FLAT_CACHE.lock().unwrap().check_blue(&path::locate_calibration_file(&c.calibration.flat_blue).unwrap()).unwrap()),
        _ => Err(constants::status::UNSUPPORTED_COLOR_CHANNEL)
    }.unwrap();
...
pub fn load_dark_file(camera:enums::Camera) -> error::Result<ImageBuffer> {
    ...
    let dark = match camera {
        enums::Camera::RED => 
                Ok(DARK_CACHE.lock().unwrap().check_red(&path::locate_calibration_file(&c.calibration.dark_red).unwrap()).unwrap()),
        enums::Camera::GREEN => 
                Ok(DARK_CACHE.lock().unwrap().check_green(&path::locate_calibration_file(&c.calibration.dark_green).unwrap()).unwrap()), 
        enums::Camera::BLUE => 
                Ok(DARK_CACHE.lock().unwrap().check_blue(&path::locate_calibration_file(&c.calibration.dark_blue).unwrap()).unwrap()),
        _ => Err(constants::status::UNSUPPORTED_COLOR_CHANNEL)
    }.unwrap();
