...
trait VectorToCylindrical {
    ...
    fn to_xy(&self, top_lat:f64, bottom_lat:f64, left_lon:f64, right_lon:f64, image_width:usize, image_height:usize) -> Point;
}
...
impl CylindricalLens {
    pub fn new(image_width:usize, image_height:usize, top_lat:f64, bottom_lat:f64, left_lon:f64, right_lon:f64) -> CylindricalLens {
        CylindricalLens { 
            image_width, 
            image_height, 
            top_lat, 
...
impl Lens for CylindricalLens {
    fn vector_to_point(&self, v:&Vector) -> Point {
        v.to_xy(self.top_lat, self.bottom_lat, self.left_lon, self.right_lon, self.image_width, self.image_height)
    }
}