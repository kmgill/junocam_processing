use junocam::{constants, enums, metadata, path, rawimage};
...
fn test_load_image() {
    ...
    assert!(path::file_exists(
        common::constants::TEST_RAW_IMAGE_FILE_PATH
    ));
    assert!(path::file_exists(common::constants::TEST_JSON_FILE_PATH));

    // Load test image
    let mut raw_image =
        rawimage::RawImage::new_from_image(common::constants::TEST_RAW_IMAGE_FILE_PATH).unwrap();
    ...
    raw_image
        .apply_hot_pixel_correction(5, 2.0)
        .expect("Error wih hot pixel correction");
    raw_image
        .apply_weights(0.9, 0.9, 0.9)
